...
function LSTMSim:train(dataset, emb_vecs)
  self.llstm:training()
  self.rlstm:training()
  if self.structure == 'bilstm' then
...
function LSTMSim:predict(lsent, rsent, emb_vecs)
  self.llstm:evaluate()
  self.rlstm:evaluate()
  local linputs = emb_vecs:index(1, lsent:long()):double()
  local rinputs = emb_vecs:index(1, rsent:long()):double()
...
function LSTMSim:predict_dataset(dataset, emb_vecs)
  local predictions = torch.Tensor(dataset.size)
  for i = 1, dataset.size do
    xlua.progress(i, dataset.size)
    local lsent, rsent = dataset.lsents[i], dataset.rsents[i]
    predictions[i] = self:predict(lsent, rsent, emb_vecs)
